...
module.exports = {
    ...
    name: "welcomer",
    description: "Set up a welcome message that gets send into a channel of your choice when a new user joins!",
    category: "server",
    cooldown: 3000,
    ...
    options: [{
        ...
        options: [{
            name: 'set',
            description: 'Picking which channel to sent the welcomer in!',
            type: ApplicationCommandOptionType.Subcommand,
            options: [{
                name: 'channel',
                description: 'The channel for the welcomer to be sent',
                required: true,
        ...
        options: [{
            name: 'set',
            description: 'Set up the embed that you want to send for the welcomer!',
            type: ApplicationCommandOptionType.Subcommand,
            options: [{
                name: 'author_text',
            ...
            name: 'reset',
            description: 'Reset your welcomer to the default welcomer',
            type: ApplicationCommandOptionType.Subcommand,
            options: [{
                name: 'confirm',
        ...
        name: 'active',
        description: 'Toggle wether or not the welcomer to be active',
        type: ApplicationCommandType.ChatInput,
        options: [{
            name: 'active',
            description: 'to active the welcomer or not',
            type: ApplicationCommandOptionType.Boolean,
            required: true,
        }]
    }, {
        name: 'test',
        description: 'to sent a message in the welcomer\'s channel',
        type: ApplicationCommandType.ChatInput
    }, {
        name: 'preview',
        description: 'Get a preview of what your welcomer looks like!',
        type: ApplicationCommandOptionType.Subcommand,
    }, {
        name: 'help',
        description: 'Get help on how to use the welcomer!',
        type: ApplicationCommandOptionType.Subcommand,
    }],
    ...
    async run({
        ...
        const defaultEmbed = {
            ...
            hexColor: `#079de8`,
            description: 'New member: <@${memberID}>\n\nWelcome! Hope you have an enjoyable at ${serverName}! Remember to follow all the rules!',
            thumbnail: {
                url: '${memberAvatar}'
            },
            image: {
                url: 'https://cdn.discordapp.com/attachments/995577242010783746/1044077033782784030/unknown.png'
            },
            author: {
                name: 'You are our ${serverMemberCount}\'th member!',
                icon_url: '${serverIcon}',
            },
            footer: {
                text: '${memberID}',
        ...
        //mongoose data
        let embed = await welcomer.findOne({
            guildId: interaction.guild.id
        }).embed || defaultEmbed

        let exist = await welcomer.findOne({
            guildId: interaction.guild.id
        })
        ...
        if (subCmdGroup === 'channel') {
            ...
            if (subCmd === 'set') {
                ...
                exist = await welcomer.findOneAndUpdate({
                    guildId: interaction.guild.id
                }, {
                    guildId: interaction.guild.id,
                ...
                try {
                    await client.channels.cache.get(options.getChannel('channel').id).send('Welcomer has been set to be sent at this channel!')
                    await interaction.reply(`The welcomer channel is now set to <#${options.getChannel('channel').id}>!\n\nYour welcomer is now active! use \`/welcomer active\` to un-active it!`)
                } catch (err) {
                    await interaction.reply('Your welcomer has been set up, but I am unable to sent messages in the channel you in which you set up the welcomer. Please make sure I have the permissions to send messages there! use \`/welcomer test\` to test if I can sent message in the welcomer channel or not!')
                }
        ...
        if (subCmdGroup === 'embed') {
            ...
            if (subCmd === 'reset') {
                ...
                if (!exist) return interaction.reply({
                    content: 'You currently don\'t have any welcomers set up!',
                    ephemeral: true
                })

                if (reset) {
                    await welcomer.findOneAndUpdate({
                        guildId: interaction.guild.id
                    }, {
                        embed: defaultEmbed
                    })
                    await interaction.reply({
                        content: 'Your welcomer has now been reset to the default welcomer.',
                        ephemeral: true
                    })
                } else {
            ...
            if (subCmd === 'set') {
                ...
                const welcomerChannelID = exist.channelId ? exist.channelId : null
                const activeWelcomer = exist.channelId ? true : false
                ...
                const welcomerChannel = exist.channelId ? `<#${exist.channelId}>` : 'None. use \`/welcomer channel set\` to set a welcomer channel!'
                const welcomerActive = exist.channelId ? '\`true\`' : '\`false\`. Use \`/welcomer active\` to active it!'

                interaction.reply({
                    content: `**Welcomer Info:**\n\n>>> Welcomer Channel: ${welcomerChannel}\nWelcomer Activated: ${welcomerActive}\n\n**Welcomer Embed Preview:**\n>>> *Don't like what you see, or made a mistake? Use \`/welcomer embed edit\` to edit a embed part, so you don't have to rebuild the whole embed!*`,
                    embeds: [
                        e
                    ],
                    ephemeral: true
            ...
            if (subCmd === 'edit') {
                ...
                const target = await welcomer.findOne({
                    guildId: interaction.guild.id
                })

                if (!target.embed) {
                    return interaction.reply({
                        content: 'You don\'t have any welcomer set up!',
                        ephemeral: true
                    })
                }
        ...
        if (subCmd === 'active') {
            ...
            if (!exist.channelId) return interaction.reply({
                content: 'You currently don\'t have any welcomer channel set up! Use \`/welcomer channel set\` to set up a channel!',
                ephemeral: true
            })
            ...
            if (active === true) {
                ...
                return interaction.reply({
                    content: 'Welcomer is now active!!',
                    ephemeral: true
                })
            } else {
                await welcomer.findOneAndUpdate({
                ...
                return interaction.reply({
                    content: 'Welcomer is now de-activated!!',
                    ephemeral: true
                })
            }
        }
        ...
        if (subCmd === 'test') {
            ...
            try {
                await client.channels.cache.get(exist.channelId).send('welcomer can be sent in this channel!')
                await interaction.reply(`Welcomer can be sent in <#${exist.channelId}>!`)
            } catch (err) {
        ...
        if (subCmd === 'preview') {
            ...
            if (exist) {
                ...
                interaction.reply({
                    content: `Welcomer Channel: <#${exist.channelId}>\nWelcomer Activated: ${welcomerActive}`,
                    embeds: [
                        e
                    ],
                    ephemeral: true
                ...
                interaction.reply('No welcomer found!')
            }
        }
        if (subCmd === 'help') {
            interaction.reply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setColor('Blurple')
                        .addFields({
                            name: '**Welcomer variables:**',
                            value: 'You can use \`none\` for a embed field value if you wish to remove it! For example, if your title field was \`hello!\`, but when you edit it you set it to \`none\`, the title will no longer appear! \n\nVariables for embed making: you can use these \"variables\" to make your welcomer better! Put the variables inside ${}, for example ${variable}, to make them work!\n\n>>> \`memberAvatar\` -- The URL of the new user\'s avatar\n\`memberName\` -- get the name of the new user (Ex: Storm7093)\n\`memberTag\` -- gets the username **and** the discriminator (Ex: Storm7093#6591)\n\`memberId\` -- ID of the new user\n\`memberCreationDay\` -- The creation day of the new user\'s account\n\`serverIcon\` -- icon URL of the server\n\`serverName\` -- name of the server\n\`serverId\` -- ID of the server\n \`serverMemberCount\` -- The current member count of the server\n\nTo switch line(go down a line/line break), just type \`\\n\` Example: Hello\\nWorld. Result: \nHello\nWorld'
                        }, {
                            name: `**Welcomer Embed Setup:**`,
                            value: `>>> The \`/welcomer channel set\` quickly sets up your welcomer, which sends the default embed at the channel of your choice.\n\nYou can use \`/welcomer embed set\` to set up your embed. Made a typo? No worries! Use the \`/welcomer embed edit\` to edit your embed without ever rebuilding it! Input \`none\` to delete that field!!\n\nUse \`/welcomer embed reset\` to reset your welcomer to the default embed!`
                        }, {
                            name: '**Others**',
                            value: '>>> \`/welcomer active\` allows you to either enable or disable your welcomer without losing your embed/message data.\n \`/welcomer test\` allows you to test if the welcomer can be sent at the channel you provided. If your welcomer doesn\'t send, try using this command.\n\`/welcomer preview\` allows you to preview your welcomer, and basic information about your current welcomer.'
                        })
                        .setFooter({
